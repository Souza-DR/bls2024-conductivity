...
module VorCells_Polygons
  ...
  PUBLIC :: Polygon,VoronoiCell,CurvilinearPolygon,voronoi_cell,voronoi_cell_old,VorCellInterConvPol, &
       VorCellInterConvPol_Collinear,interCellBall
  ...
  type Polygon
     ...
     ! number of vertices: to define a polygon, the first and the last
     ! vertices have to be the same.
     integer :: n
     ...
     ! Edge identification: each edge of a polygon can be of three
     ! types: 0 => edge is contained in the boundary of A; 1 => edge
     ! is in int(A); a negative integer X in {-nBalls,...,-1} => edge
     ! is contained in some Voronoi edge with identification given by
     ! X.
     integer, dimension(:), allocatable :: e
    
  end type Polygon
  
  ! Type for Voronoi cells. It is used when there are at least 3
  ! noncollinear generators (ball's centers).
  type VoronoiCell
    
     ! flag to indicate whether the cell is bounded or unbounded: flag
     ! = true => bounded (cell is a convex polygon).
     logical :: flag
    
     ! number of vertices: for bounded cells, the first and the last
     ...
     ! Rays: if flag = false, this array contains the origin and
     ! direction of two rays that compose unbounded cells.  [origin,
     ! origin, direction, direction], (1--3 and 2--4). If flag = true,
     ! this array will not be used.
     real(kind=8) :: rays(2,4)
    
     ! Voronoi edge identification: each Voronoi edge L (or also a ray
     ! when the cell is unbounded) of a Voronoi cell V_i is identified
     ! with a negative integer flag {-1,...,-nBalls}, that represents
     ! the index j of the center whose Voronoi cell V_j is neighbor of
     ! V_i and are separated by L.
     integer, dimension(:), allocatable :: e
    
  end type VoronoiCell
  
  ! Type for curvilinear polygons, resulting from the intersection of
  ! a convex polygon with a circle.
  type CurvilinearPolygon
    
     ! number of vertices: the first and the last vertices have to be
     ! the same.
     integer :: n
     ...
     ! Each vertex can be the intersection of a ball with: (i)
     ! boundary of A; (ii) an auxiliary edge from the partition of A
     ! in convex sets; (iii) another ball (a Voronoi edge). The
     ! identification will be done as: (i) 0; (ii) 1; (iii) a negative
     ! integer in {-nBalls, ..., -1}.  Furthermore, the vertex can be
     ! a corner of the convex polygon. In this case, we will identify
     ! the vertex as 2.
...
contains
  ...
  subroutine SortVertCell(center, vorcell)
    ! Sort the vertices of a Voronoi cell in counter-clockwise
    ! order. For unbounded cells, it is supposed to be at least three
    ! Voronoi vertices.
    ...
    if ( vorcell%flag ) then
       ...
       do ve=1,vorcell%n - 1
          angles(ve) = datan2( vorcell%v(2,ve) - center(2), vorcell%v(1,ve) - center(1) )
       end do
       ...
       do vj = 1,vorcell%n - 1
          do ve = 1,vorcell%n - 1
             if ( angles_sorted(vj) .eq. angles(ve)  ) then
                vertices_sorted(1:2,vj) = vorcell%v(1:2,ve)
             end if
          end do
       end do
       ! Store in vorcell%v.
       vorcell%v(1:2,1:vorcell%n - 1) = vertices_sorted(1:2,1:vorcell%n - 1)
       vorcell%v(1:2,vorcell%n) = vorcell%v(1:2,1)
    else
       ! First and last vertices stored in vorcell%v are also origins of the rays. 
       if ( visible( vorcell%v(1:2,2), vorcell%v(1:2,1), vorcell%v(1:2,vorcell%n) ) ) then
          ! Origin of vector is the last sorted vertex.
          do ve=2,vorcell%n
             ! Sort from the angle in radians swept out between the rays 
             ! ( vorcell%v(:,1), vorcell%v(:,ve) ) and 
             ! ( vorcell%v(:,1), vorcell%v(:,vorcell%n) ).
             angles(ve - 1) = angle_rad_2d( vorcell%v(1:2,ve), vorcell%v(1:2,1),&
                  vorcell%v(1:2,vorcell%n) )
          end do
          angles_sorted = angles
          ! Sort the angles.
          call quicksort(angles_sorted, vorcell%n - 1, 1, vorcell%n - 1)
          ...
          do vj = 2,vorcell%n
             do ve = 2,vorcell%n
                if ( angles_sorted(vj - 1) .eq. angles(ve - 1)  ) then
                   vertices_sorted(1:2,vj - 1) = vorcell%v(1:2,ve)
                end if
             end do
          end do
                
          ! Store in vorcell%v.
          last(1:2) = vorcell%v(1:2,1)
          vorcell%v(1:2,1:vorcell%n - 1) = vertices_sorted(1:2,1:vorcell%n - 1)
          vorcell%v(1:2,vorcell%n) = last(1:2)
       else
          ! Last vertex already sorted.
          do ve=1,vorcell%n - 1
             ! Sort from the angle in radians swept out between the rays 
             ! (vorcell%v(:,vorcell%n), vorcell%v(:,ve) ) and 
             ! (vorcell%v(:,vorcell%n), vorcell%v(:,1) ).
             angles(ve) = angle_rad_2d( vorcell%v(1:2,ve), vorcell%v(1:2,vorcell%n),&
                  vorcell%v(1:2,1) )
          end do
          angles_sorted = angles
          ! Sort the angles.
  ...
  ! -------------------------------------------------------- !
  subroutine voronoi_cell_destroy(vorcell)
    ...
    if ( allocated(vorcell%v) ) then
       deallocate(vorcell%v,stat=allocerr)
    ...
    if ( allocated(vorcell%e) ) then
       deallocate(vorcell%e,stat=allocerr)
       if ( allocerr .ne. 0 ) then
          write(*,*) 'Deallocation error.'
          stop
       end if
  ...
  end subroutine voronoi_cell_destroy
      
  ! -------------------------------------------------------- !
  subroutine voronoi_cell(ind_ball, nBalls, centers, start, sitetv, v_num, nod_tri, tnbr, vor_xy, vor_cell, status)
    ...
    ! ARRAYS ARGUMENTS
    integer, intent(in) :: nod_tri(3,v_num),sitetv(3*v_num),start(nBalls+1),tnbr(3,v_num) ! Delaunay triangulation data.
    ...
    ! LOCAL SCALARS
    integer :: cur_neigh,i,ind,iv,in,it,itu,j,k,l,piv,v,vprev,repeated_vertex
...
!!$          if ( ind_ball .eq. nod_tri(j,v) ) then
    ...
    do ind = start(ind_ball),start(ind_ball+1) - 1
       v = ( sitetv(ind) - 1 ) / 3 + 1
       j = sitetv(ind) - 3 * ( v - 1 )

       if ( ind .eq. start(ind_ball) ) then
          repeated_vertex = 0
       else if ( ind .ne. start(ind_ball) .and. v .ne. vprev ) then
          repeated_vertex = 0
       end if
       
       vprev = v
       
             ! Obtaining the indexes of the edges and neighbors with
             ! respect to the vertex ind_ball.
             ...
             ! When both edges jj(1) and jj(2) are also edges of
             ! another triangle (neighbor to v), then the Voronoi
             ! vertex associated to the triangle v is not an origin of
             ! a ray, that is, it is an intermediate vertex. We will
             ! distinguish the possible cases using the variable
             ! below.
...
!!$    end do
    ...
    ! Put the vor_cell%n vertices in vor_cell%v.
    if ( vor_cell%flag ) then
       vor_cell%n = iv + 1
       allocate(vor_cell%v(2,vor_cell%n))
       vor_cell%v(1:2,1:vor_cell%n - 1) = intermediate_vertices(1:2,1:iv)
       ...
       if ( repeated_origins(1) .eq. 0 .and. repeated_origins(2) .eq. 0 ) then
          vor_cell%n = iv + 2
          allocate(vor_cell%v(2,vor_cell%n))
          vor_cell%v(1:2,1) = vor_cell%rays(1:2,1)
          vor_cell%v(1:2,vor_cell%n) = vor_cell%rays(1:2,2)
          if ( iv .gt. 0 ) then
             vor_cell%v(1:2,2:vor_cell%n-1) = intermediate_vertices(1:2,1:iv)
          end if
          ...
          vor_cell%v(1:2,1) = vor_cell%rays(1:2,1)
          if ( iv .gt. 0 ) then
             vor_cell%v(1:2,2:vor_cell%n) = intermediate_vertices(1:2,1:iv)
          end if
          ...
          vor_cell%n = iv
          allocate(vor_cell%v(2,vor_cell%n))
          vor_cell%v(1:2,1:vor_cell%n) = intermediate_vertices(1:2,1:vor_cell%n)
       end if
    ...
    if ( vor_cell%flag ) then
       ...
       ! Bounded cells
       allocate(vor_cell%e(vor_cell%n - 1))
            
       do i = 1,vor_cell%n - 1
          min_absinnprod = 1.0d+20
          do itu = 1,it
             if ( r8vec_distance(2,vor_xy(:,triangles_used(itu)),vor_cell%v(:,i)) .lt. tol ) then
                do j=1,3
          ...
          vor_cell%e(i) = - cur_neigh 
                
          ! Check whether the index is correct.
          if ( .not. (1 .le. iabs(vor_cell%e(i)) .and. iabs(vor_cell%e(i)) .le. nBalls) ) then
             write(*,*) 'The edge ID is not between 1 and nBalls. (Place 1) Value = ',iabs(vor_cell%e(i))
             status = -21
             return
          end if
       ...
       min_absinnprod = 1.0d+20
       do itu = 1,it
          if ( r8vec_distance(2,vor_xy(:,triangles_used(itu)),vor_cell%v(:,1)) .lt. tol ) then
             do j=1,3
                if ( ind_ball .eq. nod_tri(j,triangles_used(itu)) ) then
                   call EdgesNeighbors(j, jj, neigh_jj)
                   exit
             ...
             if ( vor_cell%v(1,1) .eq. vor_cell%rays(1,1) .and. &
                  vor_cell%v(2,1) .eq. vor_cell%rays(2,1) ) then
                dir(:) = vor_cell%rays(:,3)
             else
                dir(:) = vor_cell%rays(:,4)
             end if
       ...
       vor_cell%e(1) = - cur_neigh
            
       ! Check whether the index is correct.
       if ( .not. (1 .le. iabs(vor_cell%e(1)) .and. iabs(vor_cell%e(1)) .le. nBalls) ) then
          write(*,*) 'The edge ID is not between 1 and nBalls. (Place 2) Value = ',iabs(vor_cell%e(1))
          status = -22
          return
       end if
       ...
       min_absinnprod = 1.0d+20
       do itu = 1,it
          if ( r8vec_distance(2,vor_xy(:,triangles_used(itu)),vor_cell%v(:,vor_cell%n)) .lt. tol ) then
             do j=1,3
                if ( ind_ball .eq. nod_tri(j,triangles_used(itu)) ) then
                   call EdgesNeighbors(j, jj, neigh_jj)
                   exit
             ...
             if ( vor_cell%v(1,vor_cell%n) .eq. vor_cell%rays(1,2) .and. &
                  vor_cell%v(2,vor_cell%n) .eq. vor_cell%rays(2,2) ) then
                dir(:) = vor_cell%rays(:,4)
             else
                dir(:) = vor_cell%rays(:,3)
             end if
       ...
       vor_cell%e(vor_cell%n + 1) = - cur_neigh
            
       ! Check whether the index is correct.
       if (.not. (1 .le. iabs(vor_cell%e(vor_cell%n + 1)) .and. iabs(vor_cell%e(vor_cell%n + 1)) .le. nBalls)) then
          write(*,*) 'The edge ID is not between 1 and nBalls. (Place 3) Value = ',iabs(vor_cell%e(vor_cell%n + 1))
          status = -23
          return
       end if
       ...
       if ( vor_cell%n .ge. 2 ) then
          do i = 1,vor_cell%n - 1
                ...
                if ( r8vec_distance(2,vor_xy(:,triangles_used(itu)),vor_cell%v(:,i)) .lt. tol ) then
                   ...
                   do l=1,2
                      absinnprod = dabs(dot_product(centers(:,nod_tri(neigh_jj(l),triangles_used(itu)))&
                           -centers(:,ind_ball),vor_cell%v(:,i+1)-vor_cell%v(:,i)))
                      if ( absinnprod .lt. min_absinnprod ) then
                         min_absinnprod = absinnprod
                         cur_neigh = nod_tri(neigh_jj(l),triangles_used(itu))
                      end if
             ...
             vor_cell%e(i+1) = - cur_neigh
                    
             ! Check whether the index is correct.
             if ( .not. (1 .le. iabs(vor_cell%e(i+1)) .and. iabs(vor_cell%e(i+1)) .le. nBalls) ) then
                write(*,*) 'The edge ID is not between 1 and nBalls. (Place 4) Value = ',iabs(vor_cell%e(i+1))
                status = -24
                return
             end if
  ...
  ! -------------------------------------------------------- !
  subroutine voronoi_cell_old(ind_ball, nBalls, centers, v_num, nod_tri, tnbr, vor_xy, vor_cell, status)
    ! Obtain the Voronoi cell (vor_cell) of the generator (ball center) ind_ball.
  ...
  end subroutine voronoi_cell_old
    
  ! -------------------------------------------------------- !
  subroutine EdgesNeighbors(j, jj, neigh_jj)
    ! Given any triangle with vertices nod_tri(j,:), j=1,2, or 3, this
    ! routine computes the indices jj of the edges that contain the
    ! vertex with index j. Also, computes the indexes of each neighbor
    ! vertex of nod_tri(j,:) in jj.
  ...
  ! -------------------------------------------------------- !
  subroutine polygon_copy(pol,polcopy)
      
    implicit none
        
    type(Polygon), intent(inout) :: pol,polcopy
      
    integer :: allocerr

    call polygon_destroy(polcopy)
    
    polcopy%deg = pol%deg

    if ( .not. polcopy%deg ) then
       polcopy%n = pol%n
       
       allocate(polcopy%v(2,polcopy%n),polcopy%e(polcopy%n-1),stat=allocerr)
       if ( allocerr .ne. 0 ) then
          write(*,*) 'Allocation error.'
          stop
       end if

       polcopy%v(1:2,1:polcopy%n) = pol%v(1:2,1:polcopy%n)
       polcopy%e(1:polcopy%n-1) = pol%e(1:polcopy%n-1)
    end if
       
  end subroutine polygon_copy
    
  ! -------------------------------------------------------- !
  subroutine polygon_destroy(pol)
    ...
    if ( allocated(pol%v) ) then
       deallocate(pol%v,stat=allocerr)
       if ( allocerr .ne. 0 ) then
          write(*,*) 'Deallocation error.'
          stop
       end if
    ...
    if ( allocated(pol%e) ) then
       deallocate(pol%e,stat=allocerr)
       if ( allocerr .ne. 0 ) then
          write(*,*) 'Deallocation error.'
          stop
       end if
  ...
  subroutine VorCellInterConvPol(center, convPol, vor_cell, cell)
    ! This subroutine computes the intersection of a convex polygon
    ! (convPol) with a Voronoi cell (vor_cell), returning a convex
    ! polygon (cell).
    ...
    allocate(cell%v(2, convPol%n + vor_cell%n + 2))
    allocate(cell%e(convPol%n + vor_cell%n + 1))
    ...
    if ( vor_cell%flag ) then
       ...
       allocate( clipping_window_cell%v(2,aux), clipping_window_cell%e(vor_cell%n + 1) )
       clipping_window_cell%v(:,2:aux - 1) = vor_cell%v(:,1:vor_cell%n)
       clipping_window_cell%e(1:vor_cell%n + 1) = vor_cell%e(1:vor_cell%n + 1)
       ...
       if ( vor_cell%n .eq. 1 ) then ! One vertex => two half-lines.
          ...
          if ( visible( center, clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,3),&
               clipping_window_cell%v(:,2) ) ) then
             clipping_window_cell%v(:,1) = clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,3)
             clipping_window_cell%v(:,aux) = clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,4)
          else
             clipping_window_cell%v(:,1) = clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,4)
             clipping_window_cell%v(:,aux) = clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,3)
             ! Correct the order of edge identification.
             aux_edge = clipping_window_cell%e(1)
             clipping_window_cell%e(1) = clipping_window_cell%e(2)
             clipping_window_cell%e(2) = aux_edge
          end if
          ! Compute the intersection.
          ...
          if ( visible( center, clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,3),&
               clipping_window_cell%v(:,2) ) ) then
             clipping_window_cell%v(:,1) = clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,3)
             clipping_window_cell%v(:,aux) = clipping_window_cell%v(:,aux - 1) + clipping_window_cell%rays(:,4)
          else
             aux_vec(1:2) = clipping_window_cell%v(:,2)
             clipping_window_cell%v(:,1) = clipping_window_cell%v(:,aux - 1) + clipping_window_cell%rays(:,4)
             clipping_window_cell%v(:,aux) = clipping_window_cell%v(:,2) + clipping_window_cell%rays(:,3)
             clipping_window_cell%v(:,2) = clipping_window_cell%v(:,aux - 1)
             clipping_window_cell%v(:,aux - 1) = aux_vec(1:2)
             ! Correct the order of edge identification.
             aux_edge = clipping_window_cell%e(1)
             clipping_window_cell%e(1) = clipping_window_cell%e(3)
             clipping_window_cell%e(3) = aux_edge
          end if
          call SutherlandHodgman( clipping_window_cell, convPol, cell )
          ...
          if ( r8vec_distance(2,clipping_window_cell%v(:,2),clipping_window_cell%rays(:,1)) .le. tol ) then
             clipping_window_cell%v(:,1) = clipping_window_cell%rays(:,1) + clipping_window_cell%rays(:,3)
             clipping_window_cell%v(:,aux) = clipping_window_cell%rays(:,2) + clipping_window_cell%rays(:,4)
          else
             clipping_window_cell%v(:,1) = clipping_window_cell%rays(:,2) + clipping_window_cell%rays(:,4)
             clipping_window_cell%v(:,aux) = clipping_window_cell%rays(:,1) + clipping_window_cell%rays(:,3)
          end if
          ! Compute the intersection.
          call SutherlandHodgman( clipping_window_cell, convPol, cell )
       end if
       deallocate( clipping_window_cell%v, clipping_window_cell%e )
    end if
  ...
  subroutine VorCellInterConvPol_Collinear(ind_ball, nBalls, centers, convPol, cell)
    ! This subroutine computes the intersection of a convex polygon
    ! (convPol) with the Voronoi cell ind_ball when the generators
    ! (ball's centers) are collinear. Return: a convex polygon (cell).
    ...
    clipping_window_cell%flag = .false.
    allocate(clipping_window_cell%v(2,clipping_window_cell%n))
    ...
    if ( neighbor .eq. 0 .or. neighbor2 .eq. 0 ) then
       ...
       if ( visible( centers(:,ind_ball), midpoint, midpoint(1:2) + normal(1:2) ) ) then
          clipping_window_cell%v(:,1) = midpoint(1:2)
          clipping_window_cell%v(:,2) = midpoint(1:2) + normal(1:2)
       else
          clipping_window_cell%v(:,1) = midpoint(1:2) + normal(1:2)
          clipping_window_cell%v(:,2) = midpoint(1:2)
       end if
       ! Adding the edge (line) identification.
       allocate(clipping_window_cell%e(1))
       clipping_window_cell%e(1) = - neighbor
       ! Allocate the maximum number of vertices and edges in cell.
       allocate(cell%v(2, convPol%n + clipping_window_cell%n))
       allocate(cell%e(convPol%n + clipping_window_cell%n - 1))
       ...
       ! Adding the edge (line) identification.
       allocate(clipping_window_cell%e(2))
       clipping_window_cell%e(1) = - neighbor
       ! Allocate the maximum number of vertices and edges in aux_pol.
       allocate(aux_pol%v(2, convPol%n + clipping_window_cell%n))
       allocate(aux_pol%e(convPol%n + clipping_window_cell%n - 1))
       ...
       ! Adding the edge (line) identification.
       clipping_window_cell%e(2) = - neighbor2
       ! Allocate the maximum number of vertices and edges in cell.
       allocate(cell%v(2, aux_pol%n + clipping_window_cell%n))
       allocate(cell%e(convPol%n + clipping_window_cell%n - 1))
       ! Intersection of aux_pol with the second half-plane.
       call SutherlandHodgman(clipping_window_cell, aux_pol, cell)
       deallocate(clipping_window_cell%v, clipping_window_cell%e, aux_pol%v)
    end if
  ...
  subroutine interCellBall(center, r, cell, curv_pol)
    ! This subroutine computes the intersection of a convex polygon
    ! (cell) with the ball B(center, r).  Return: a curvilinear
    ! polygon (curv_pol).
    ...
    allocate( curv_pol%v( 2, 2 * cell%n ) )
    allocate( curv_pol%vertex_id( 2 * cell%n ) )
    allocate( curv_pol%e( 2 * (cell%n - 1) ) )
    ...
    do i = 1,cell%n - 1
       ...
       d = r8vec_distance(2, cell%v(1:2,i), cell%v(1:2,i+1))
       if ( d .lt. tol) then
          go to 10
       end if
            
       ! Intersection between circle and line segment with endpoints cell%v(1:2,i) and cell%v(1:2,i+1).
       call circle_imp_segment_intersect ( r, center, cell%v(1:2,i), cell%v(1:2,i+1), int_num, p )
       ...
       if ( int_num .eq. 2 ) then
          ...
          d1 = r8vec_distance ( 2, cell%v(:,i), p(:,1) )
          d2 = r8vec_distance ( 2, cell%v(:,i), p(:,2) )
          if ( d1 .lt. d2 ) then
             iv = iv + 1
             curv_pol%v(1:2,iv) = p(1:2,1)
             curv_pol%vertex_id(iv) = cell%e(i)
             iv = iv + 1
             curv_pol%v(1:2,iv) = p(1:2,2)
             curv_pol%vertex_id(iv) = cell%e(i)
          else
             iv = iv + 1
             curv_pol%v(1:2,iv) = p(1:2,2)
          ...
          curv_pol%vertex_id(iv) = cell%e(i)
          last_added = 1
          if ( (cell%v(1,i+1) - center(1))**2 + (cell%v(2,i+1) - center(2))**2 .lt. r**2 ) then
             iv = iv + 1
             curv_pol%v(1:2,iv) = cell%v(1:2,i+1)
...
10     continue  
    ...
    if (curv_pol%n .eq. 0) then
       ...
       iv = iv + 1
       curv_pol%n = iv
       curv_pol%v(1:2,iv) = curv_pol%v(1:2,1)
  ...
  subroutine SutherlandHodgman( W, P, Q )
    ...
    allocate( workPolygon%v( 2, W%n + P%n ), workPolygon%e(W%n + P%n - 1) )
        
    ! initialize the work polygon with P.
    workPolygon%n = P%n
    workPolygon%v(:,1:workPolygon%n) = P%v(:,1:workPolygon%n)
    workPolygon%e(1:W%n + P%n - 1) = 111
    workPolygon%e(1:workPolygon%n - 1) = P%e(1:workPolygon%n - 1)
    ...
    do i=1,W%n - 1
       ...
       call edgeClipping( workPolygon, y1, y2, W%e(i), Q )
       workPolygon%n = Q%n
       workPolygon%v(:,1:workPolygon%n) = Q%v(:,1:workPolygon%n)
       if ( workPolygon%n .gt. 0 ) then
          workPolygon%e(1:workPolygon%n - 1) = Q%e(1:workPolygon%n - 1)
       end if
    end do
    ...
    if ( Q%n .gt. 3 ) then
       ...
       call polygon_1_2d ( Q%n - 1, Q%v(1:2,1:Q%n - 1), res )
       if ( abs(res) .le. tol ) then
          Q%deg = .true.
       end if
    end if
    
    deallocate(workPolygon%v)
  ...
  subroutine edgeClipping( poly, y1, y2, id_vor_edge, outputPoly )
    ...
    if ( d .le. tol ) then
       outputPoly%n = poly%n
       outputPoly%v(:,1:outputPoly%n) = poly%v(:,1:outputPoly%n)
       outputPoly%e(1:outputPoly%n - 1) = poly%e(1:outputPoly%n - 1)
       return
    end if
    
    do i = 1,poly%n-1 ! for each edge i of poly
       x1(:) = poly%v(:,i)   ! vertex 1 of edge i
       x2(:) = poly%v(:,i+1) ! vertex 2 of edge i
       if ( visible(x1, y1, y2) ) then ! if x1 is visible
          if ( visible(x2, y1, y2) ) then ! if x2 is visible
             ! add x2 to the output polygon
             if ( c .eq. 0 .or. (c .gt. 0 .and. r8vec_distance(2,outputPoly%v(:,c),x2) .gt. tol) ) then
                c = c+1
                outputPoly%v(:,c) = x2(:)
                outputPoly%e(c) = poly%e(i)
             end if
             ...
             if ( c .eq. 0 .or. (c .gt. 0 .and. r8vec_distance(2,outputPoly%v(:,c),intersecPoint) .gt. tol) ) then
                c = c+1
                outputPoly%v(:,c) = intersecPoint(:)
                outputPoly%e(c) = poly%e(i)
             end if
          end if
          ...
          if ( visible(x2, y1, y2) ) then ! x2 is visible
             ...
             if ( .not. intsct ) then
                ...
                if ( c .eq. 0 .or. (c .gt. 0 .and. r8vec_distance(2,outputPoly%v(:,c),intersecPoint) .gt. tol) ) then
                   c = c+1
                   outputPoly%v(:,c) = intersecPoint(:)
                   outputPoly%e(c) = id_vor_edge
                end if
                if ( r8vec_distance(2,outputPoly%v(:,c),x2) .gt. tol ) then
                   c = c+1
                   outputPoly%v(:,c) = x2(:)
                   outputPoly%e(c) = poly%e(i)
                end if
    ...
    if (c .gt. 0) then
       last_edge = outputPoly%e(1)
       outputPoly%e(1:c-1) = outputPoly%e(2:c)
       outputPoly%e(c) = last_edge
    end if
        
    ! Last and first vertices must be the same.
    if (c .gt. 0) then
       d = r8vec_distance(2,outputPoly%v(:,1),outputPoly%v(:,c))
       if ( d .gt. tol ) then
          c = c+1
          outputPoly%v(:,c) = outputPoly%v(:,1)
       end if
    end if
  ...
  subroutine intersection( x1, x2, y1, y2, inter, intsct )
    ! It computes the intersection (if it exists) between a segment
    ! [x1,x2] and the line passing through y1 and y2.
  ...
  function visible( p, y1, y2 )
    ! This function checks whether p belongs to the half-plane to the
    ! left of line (y1--y2).  (i.e., p is "visible"?).
